...
class DataExplorer:
    def __init__(
        ...
        if df is None and loader is None:
            self.logger.error("No DataFrame or DataLoader provided")
            raise ValueError("Provide either a DataFrame or a DataLoader")
        self._df = df
        self.loader = loader
    ...
    def df(
        ...
        if self._df is None:
            if self.loader is None:
                ...
                raise RuntimeError("No data available and no loader configured")
            if hasattr(self, "logger"):
    ...
    def reload(
        ...
        if self.loader is None:
            ...
            raise RuntimeError("Cannot reload without a DataLoader")
        if hasattr(self, "logger"):
            self.logger.info(f"Reloading data (force={force})")
        self._df = self.loader.load_data(force=force)
        if hasattr(self, "logger"):
