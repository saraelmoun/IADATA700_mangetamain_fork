...
class TestIngredientsClusteringConfig:
    ...
    def test_config_path_types(self):
        ...
        config2 = IngredientsClusteringConfig(recipes_path="recipes.csv")
...
class TestIngredientsClusteringPage:
    ...
    def sample_recipes_data(self):
        ...
        common_ingredients = [
            ...
            'onion', 'garlic', 'olive oil', 'tomato', 'cheese', 'chicken',
            'beef', 'rice', 'pasta', 'carrot', 'potato', 'lemon', 'basil'
        ]
        ...
        for _ in range(n_recipes):
            ...
            recipe_ingredients = list(np.random.choice(
                common_ingredients, 
                size=n_ingredients, 
                replace=False
            ))
    ...
    def test_render_sidebar_returns_expected_structure(self, page_instance):
        ...
        with patch('streamlit.sidebar.header'), \
            ...
            mock_slider.side_effect = [50, 5, 30]  # n_ingredients, n_clusters, perplexity
            mock_button.return_value = False
            
            params = page_instance.render_sidebar()
            
            # Vérifier la structure du dictionnaire retourné
            expected_keys = ['n_ingredients', 'n_clusters', 'tsne_perplexity', 'analyze_button']
    ...
    def test_render_analysis_summary(self, page_instance):
        ...
        mock_analyzer.debug_ingredient_mapping.return_value = {
            'search_results': {
                'pepper': [{'ingredient': 'black pepper', 'is_representative': True, 'representative': 'pepper'}],
                'egg': [{'ingredient': 'large eggs', 'is_representative': False, 'representative': 'egg'}]
            }
        }
        ...
        mock_analyzer.get_processing_summary.return_value = {
            'input_data': {'total_recipes': 1000, 'total_raw_ingredients': 5000, 'avg_ingredients_per_recipe': 5},
            'normalization': {'total_unique_raw': 2000, 'total_normalized': 500, 'reduction_ratio': 75},
            'grouping': {'groups_with_multiple_items': 150, 'largest_group_size': 10},
            'cooccurrence_matrix': {'dimensions': '50x50', 'total_cooccurrences': 1250, 'non_zero_pairs': 800, 'sparsity': 36}
        }
    ...
    def test_formal_language_in_methods(self, page_instance):
        ...
        for method in [page_instance.run, page_instance.render_sidebar, 
...
class TestIngredientsClusteringPageTyping:
    ...
    def test_return_type_annotations(self):
        ...
        for method_name in ['render_cooccurrence_analysis', 'render_clusters', 
                           'render_sidebar_statistics', 'render_analysis_summary', 'run']:
            method = getattr(page, method_name)
...
class TestDocumentation:
    ...
    def test_all_public_methods_documented(self):
        ...
        public_methods = [
            name for name in dir(page)
            if callable(getattr(page, name)) and not name.startswith('_')
        ]
        
        for method_name in public_methods:
            method = getattr(page, method_name)
            assert method.__doc__ is not None, \
                f"La méthode {method_name} n'a pas de docstring"
            assert len(method.__doc__.strip()) > 20, \
                f"La docstring de {method_name} est trop courte"
